...
- [Visualizing EEM data](#visualize)
    - [`drawEEM`](#drawEEM)
    - [`drawEEMgg`](#drawEEMgg)
    - [`drawMultipleEEM`](#drawMultipleEEM)
- [Methods for EEM objects]
    - [Joining data](#commonize)
    - [Combining data of different dimensions](#combine)
    - [Applying functions on EEM objects](#applyEEM)
...
- [Misc]
    - [converting objects between EEM and eemR package](#eemlist2EEM)
- [References](#ref)

## <a name="intro"></a>Introduction
Fluorescence fingerprint or more commonly known as complete fluorescence excitation-emission matrix (EEM) is a 3-dimensional data consisting of excitation, emission and intensity axis. The multi-dimension set EEM data apart from other signal processing. Thus, `EEM` package was developed to facilitate data import and analysis in R. Basic tools for importing raw data files, deleting Rayleigh scattering rays, unfolding 3-dimensional to 2-dimentional matrix (and vice versa) for further multivariate analysis, visualizing data, etc. are provided in this package. The author has intended this package to be used as a bridge between raw data files and other analysis packages. 

## <a name="import"></a>Importing raw data files
`readEEM` function can read raw data files into R. Currently the supported raw data files are limited to certain fluorescence spectrometers (see `?readEEM` or [importing raw files vignette](https://github.com/chengvt/EEM/blob/master/vignettes/file-io.md) for more details). Basically `readEEM` will look for a certain keyword in raw files and start to read in the lines below them. Please send a word or pull request to add support for other formats. 
...
The data is imported as a `list` and was given a class name of `EEM`. The original file names are can be retrieved using `names(EEM)`, and additional information can be obtained using `summary(data)`. In each list element, there is a matrix where row names are numerical value specifying emission wavelength and column names are excitation wavelength. The values are intensities at each combination of excitation and emission wavelength. 
...
applejuice[[1]][1:5, 1:5]
```

For demonstation purpose, an "applejuice" dataset is attached with the package. It can be called by `data(applejuice)`. More information about the dataset can be accessed by `?applejuice`. 
...
EEM data is usually visualized using a contour representation. The functions for creating contours easily for `EEM` class object are explained below. 
...
### <a name="drawMultipleEEM"></a>drawMultipleEEM
`drawMultipleEEM` draws all elements in an `EEM` object. 

```{r}
# draw sample 1 to 6
drawMultipleEEM(applejuice[1:6]) 

# make three columns instead
drawMultipleEEM(applejuice[1:6], ncol = 3) 
```

## Methods for EEM object

### <a name="commonize"></a>Joining data
`c.EEM` is provided to provide an easier way to combine EEM datasets.

```{r}
# call another set of data
data(gluten)

# combine with applejuice data
combined_data <- c(gluten, applejuice)
class(combined_data)
```

### <a name="combine"></a>Combining data of different dimensions

Sometimes, it is possible to have some raw data files with different dimensions of wavelength. For example, the first file may contain emission wavelength from 200 to 900 nm, while another file contains emission wavelength from 300 to 500 nm. The import function `readEEM` has no problems with data of different dimensions. However, this is not tolerated by `unfold` which requires all objects in EEM object have the same dimensions. This can be solved by applying `commonizeEEM` prior to `unfold`.

```{r}
summary(combined_data) # different dimensions
data_combined <- commonizeEEM(combined_data)
summary(data_combined) # same dimension, ready for unfold
```

The basic idea is that `commonizeEEM` will only select the common variables of all data in EEM object. The figure below illustrates how `commonizeEEM` select only the common region.

![](photos/commonize.png)

### <a name="applyEEM"></a> Applying functions on EEM object
`applyEEM` can be used to apply basic functions to all numeric elements. The merit of this function is that it will unfold the EEM object, apply the function, and fold it back into EEM object. Some recommended usages are logarithmatic and arithmetic  transformation.
...
## <a name="eemlist2EEM"></a>Converting objects between EEM and eemR package(#eemlist2EEM)
`eemlist2EEM` converts from `eemlist` object of [eemR package](https://cran.r-project.org/web/packages/eemR/index.html) into `EEM` object of EEM package. `EEM2eemlist` does vice versa.
